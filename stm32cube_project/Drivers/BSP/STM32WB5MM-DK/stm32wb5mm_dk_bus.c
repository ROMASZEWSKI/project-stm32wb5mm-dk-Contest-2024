/**
  ...
  *
  ******************************************************************************
  * Modifications 2023 - A. Romaszewski Add drivers for i2c1
  */
...
static uint32_t I2c21InitCounter = 0;
#if (USE_HAL_I2C_REGISTER_CALLBACKS == 1)
static uint32_t Bus_IsI2c2MspCbValid = 0;
static uint32_t Bus_IsI2c21MspCbValid = 0;
...
static int32_t  I2C1_Read(uint16_t DevAddr, uint8_t *pData, uint16_t Length);
...
int32_t BSP_I2C1_Init(void)
{
  ...
  if (I2c21InitCounter < 0xFFFFFFFFU)
  {
    I2c21InitCounter++;
  }
  return status;
...
{
  return I2C1_Read(DevAddr, pData, Length);
}
...
static int32_t I2C1_Read(uint16_t DevAddr, uint8_t *pData, uint16_t Length)
{
	int32_t  status = BSP_ERROR_NONE;
	uint32_t hal_error;
	if (HAL_I2C_Master_Receive(&hbus_i2c1, DevAddr, pData, Length, BUS_I2C3_TIMEOUT) != HAL_OK)
	{
		hal_error = HAL_I2C_GetError(&hbus_i2c1);
		if ((hal_error & HAL_I2C_ERROR_BERR) != 0U)
		{
